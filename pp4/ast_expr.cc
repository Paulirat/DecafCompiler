...
Type* ArithmeticExpr::TypeFinder() {
    ...
    if (left == NULL) {
        if (rtype->AreEquivalent(Type::intType) ||
            rtype->AreEquivalent(Type::doubleType))
            return rtype;
        else
            return Type::errorType;
    }
    ...
    if (ltype->AreEquivalent(Type::intType) &&
        rtype->AreEquivalent(Type::intType))
        return ltype;

    if (ltype->AreEquivalent(Type::doubleType) &&
        rtype->AreEquivalent(Type::doubleType))
        return ltype;
...
Type* EqualityExpr::TypeFinder() {
    ...
    if (!rtype->AreEquivalent(ltype) &&
        !ltype->AreEquivalent(rtype))
        return Type::errorType;
...
Type* LogicalExpr::TypeFinder() {
    ...
    if (left == NULL) {
        if (rtype->AreEquivalent(Type::boolType))
            return Type::boolType;
        else
            return Type::errorType;
    }
    ...
    if (ltype->AreEquivalent(Type::boolType) &&
        rtype->AreEquivalent(Type::boolType))
        return Type::boolType;
...
void AssignExpr::Check() {
    ...
    if (!rtype->AreEquivalent(ltype) && !ltype->IsEquivalentTo(Type::errorType))
        ReportError::IncompatibleOperands(op, ltype, rtype);
}
...
void ArrayAccess::Check() {
    ...
    if (subscript->TypeFinder() != Type::errorType && !subscript->TypeFinder()->IsEquivalentTo(Type::intType))
...
void NewArrayExpr::Check() {
    ...
    if (size->TypeFinder() != Type::errorType && !size->TypeFinder()->IsEquivalentTo(Type::intType))
        ReportError::NewArraySizeNotInteger(size);

    if (elemType->Primitive() && !elemType->AreEquivalent(Type::voidType))
        return;

    Decl *d = Program::G_Scope->table->Lookup(elemType->Name());
    if (dynamic_cast<ClassDecl*>(d) == NULL)
        elemType->UndeclaredId(LookingForType);
}
