...
bool Decl::AreEquivalent(Decl *other) {
    return true;
}
...
bool VarDecl::AreEquivalent(Decl *other) {
    VarDecl *varDecl = dynamic_cast<VarDecl*>(other);
    if (varDecl == NULL)
        return false;

    return type->AreEquivalent(varDecl->type);
}
    ...
    while (s != NULL) {
        ...
        if ((d = s->table->Lookup(type->Name())) != NULL) {
            if (dynamic_cast<ClassDecl*>(d) == NULL &&
                ...
                type->UndeclaredId(LookingForType);
...
void ClassDecl::ExtendsFinder() {
    ...
    if (dynamic_cast<ClassDecl*>(lookup) == NULL)
        extends->UndeclaredId(LookingForClass);
}
...
void ClassDecl::ImplementsFinder() {
    ...
    for (int i = 0, n = implements->NumElements(); i < n; ++i) {
        ...
        if (dynamic_cast<InterfaceDecl*>(lookup) == NULL)
            nth->UndeclaredId(LookingForInterface);
    }
}
...
bool FnDecl::AreEquivalent(Decl *other) {
    FnDecl *fnDecl = dynamic_cast<FnDecl*>(other);

    if (fnDecl == NULL)
        return false;

    if (!returnType->AreEquivalent(fnDecl->returnType))
        return false;
    ...
    for (int i = 0, n = formals->NumElements(); i < n; ++i)
        if (!formals->Nth(i)->AreEquivalent(fnDecl->formals->Nth(i)))
            return false;
